...
@push('js')
    ...
    <script type="text/javascript">
        ...
        function store(){
            $('#formAction').on('submit', function(e){
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: function(res) {
                        window.LaravelDataTables["class-table"].ajax.reload();
                        modal.hide();
                        Toast.fire({
                            icon: res.status,
                            title: res.message
        ...
        $('#class-table').on('click', '.action', function(){
            let data = $(this).data();
            let id = data.id;
            let type = data.type;
            ...
            if(type == 'edit') {
                $.ajax({
                    url: '{{ route('classes.edit', ':id') }}'.replace(':id', id),
                    type: 'GET',
                    success: function(res) {
                        $('#modalAction').find('.modal-dialog').html(res);
                        modal.show();
            ...
            if(type == 'delete') {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ route('classes.destroy', ':id') }}`.replace(':id', id),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN' : $('meta[name="csrf-token"]').attr('content')
                            },
            ...
            if(type == 'detail'){
                $.ajax({
                    url: `{{ route('classes.show', ':id') }}`.replace(':id', id),
                    type: 'GET',
                    success: function(res) {
                        $('#modalAction').find('.modal-dialog').html(res);
                        modal.show();
