...
@push('js')
    ...
    <script type="text/javascript">
        ...
        function store(){
            $('#formAction').on('submit', function(e){
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: function(res) {
                        window.LaravelDataTables["teacher-table"].ajax.reload();
                        modal.hide();
                        Toast.fire({
                            icon: res.status,
                            title: res.message
        ...
        $('#teacher-table').on('click', '.action', function(){
            let data = $(this).data();
            let id = data.id;
            let type = data.type;
            if(type == 'delete'){
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            method: 'DELETE',
                            url: '{{ route('teachers.destroy', ':id') }}'.replace(':id', id),
                            headers: {
                                'X-CSRF-TOKEN' : $('meta[name="csrf-token"]').attr('content')
                            },
                            success: function(res) {
            ...
            if(type == 'edit'){
                $.ajax({
                    url: `{{ route('teachers.edit', ':id') }}`.replace(':id', id),
                    type: 'GET',
                    success: function(res) {
                        $('#modalAction').find('.modal-dialog').html(res);
                        modal.show();
            ...
            if(type == 'detail'){
                $.ajax({
                    url: `{{ route('teachers.show', ':id') }}`.replace(':id', id),
                    type: 'GET',
                    success: function(res) {
                        $('#modalAction').find('.modal-dialog').html(res);
                        modal.show();
